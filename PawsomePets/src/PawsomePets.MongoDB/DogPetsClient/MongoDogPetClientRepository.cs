...
{
    public class MongoDogPetClientRepository : MongoDbRepository<PawsomePetsMongoDbContext, DogPetClient, int>, IDogPetClientRepository
    {
        public MongoDogPetClientRepository(IMongoDbContextProvider<PawsomePetsMongoDbContext> dbContextProvider)
        ...
        public virtual async Task<List<DogPetClient>> GetListAsync(
            string? filterText = null,
            string? name = null,
            string? breed = null,
            float? ageMin = null,
        ...
        {
            var query = ApplyFilter((await GetMongoQueryableAsync(cancellationToken)), filterText, name, breed, ageMin, ageMax, gender, color, weightMin, weightMax, healthStatus, vaccinationsMin, vaccinationsMax, priceMin, priceMax, promotionPecentsMin, promotionPecentsMax, isStock, otherInformation);
            query = query.OrderBy(string.IsNullOrWhiteSpace(sorting) ? DogPetClientConsts.GetDefaultSorting(false) : sorting);
            return await query.As<IMongoQueryable<DogPetClient>>()
                .PageBy<DogPetClient, IMongoQueryable<DogPetClient>>(skipCount, maxResultCount)
        ...
        {
            var query = ApplyFilter((await GetMongoQueryableAsync(cancellationToken)), filterText, name, breed, ageMin, ageMax, gender, color, weightMin, weightMax, healthStatus, vaccinationsMin, vaccinationsMax, priceMin, priceMax, promotionPecentsMin, promotionPecentsMax, isStock, otherInformation);
            return await query.As<IMongoQueryable<DogPetClient>>().LongCountAsync(GetCancellationToken(cancellationToken));
        }

        protected virtual IQueryable<DogPetClient> ApplyFilter(
            IQueryable<DogPetClient> query,
            string? filterText = null,
            string? name = null,
            string? breed = null,
            float? ageMin = null,
